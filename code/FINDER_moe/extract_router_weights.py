...
def main():
    ...
    # Update model config based on path
    # Parse model path like "MoE_BA_nrange_50_100_m_6"
    if 'MoE_BA' in args.model_path:
    # Extract parameters from path
        path_parts = args.model_path.split('_')
        nrange = None
        m = None
    
    for i, part in enumerate(path_parts):
        if part == 'nrange' and i + 1 < len(path_parts):
            nrange = path_parts[i + 1] + '_' + path_parts[i + 2]
        elif part == 'm' and i + 1 < len(path_parts):
            m = int(path_parts[i + 1])
